...
-- By eyeballing the dataset and doing ad hoc analysis, it appears the dataset also contains 'video games', 'straight to video prodcutions' and 'TV series'
-- These must be filtered out in our analysis
...
-- Creating View for Poer BI which filters out all 'video games', 'straight to video prodcutions' and 'TV series'
-- Therefore only Productions with at least one million budget are considered
...
SELECT AVG(budget_to_revenue_ratio)
FROM movies_analytics_final
WHERE revenue > 0
AND budget > 1000000
...
-- Viewing the 'main production companys' with a big budget
SELECT DISTINCT(main_production_company)
FROM movies_analytics_final
WHERE revenue > 0
AND budget > 100000000;


-- Viewing revenue, budget, average runtime and average rating in millions for every production company
SELECT 
...
-- Viewing revenue, budget, average runtime and average rating in millions for every main genre
SELECT 
	ROUND(SUM(revenue) / 1000000, 4) AS total_revenue_millions,
    ROUND(SUM(budget) / 1000000, 4) AS total_budget_millions,
    ROUND(AVG(runtime)::numeric, 2) AS average_runtime,
